...
public class ExamManageController {
    ...
    public ApiResult getAllTopics() {
        List<ExamManage> all = examManageService.findAll();
        List<JSONObject> res = new ArrayList<>();
        for (ExamManage em : all) {
            SubjectSumVo sumBySubject = examManageService.findSumBySubject(em.getSource());
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("subjectId",em.getExamCode());
            jsonObject.put("subjectName",em.getSource());
            jsonObject.put("fillNum",sumBySubject.getFillNum());
            jsonObject.put("judgeNum",sumBySubject.getJudgeNum());
            jsonObject.put("multiNum",sumBySubject.getMultiNum());
            res.add(jsonObject);
        }
        return  new ApiResult(200,"OK",res);
    }
